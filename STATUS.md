...
| 5b | Datenbank-Entscheidung | ✅ Fertig | Bestätigt: Mitspieler-Namen verlassen das Gerät über den Supabase-Ranking-Pfad nicht (siehe Präzisierung zu Audit-Runde 2 in `AUDIT.md`, Fund B9 in `AUDIT-2.md` — bei aktiver Google-Stimme/Vokabel-KI gilt das nicht generell). SQL-Bug gefunden+gefixt (doppelte `close_month`-Funktion). Migrations-Workflow eingerichtet. **Offen bei euch:** Supabase-CLI verlinken. |
